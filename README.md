...
├── data/
│   ├── df_before.csv         ← not tracked (700MB) — download from Kaggle
│   ├── df_clean.csv          ← not tracked (200MB) — generated by notebook
│   └── df_train.csv          ← not tracked (200MB) — generated by notebook
└── models/
...
> ⚠️ `rf_model_sm.pkl` not tracked (file too large). Random Forest metrics are included in comparison charts via hardcoded results from the notebook.
```
...
Raw data is available on Kaggle:  
🔗 [IEEE-CIS Fraud Detection Dataset](https://www.kaggle.com/c/ieee-fraud-detection/data)

Download `train_transaction.csv` and `train_identity.csv`, place them in a `raw/` folder, then run the notebook top to bottom — it will generate `df_before.csv`, `df_clean.csv`, and `df_train.csv` automatically in the `data/` folder.
